...
class Portfolio:
    ...
    def optimize(self) -> Tuple[float, List[float]]:
        total_returns = [
            sum(asset.returns[i] for asset in self.assets)
            for i in range(len(self.assets[0].returns))
        ]
        avg_returns = [
            total_return / len(self.assets) for total_return in total_returns
        ]
        expected_return = sum(avg_returns)
        return expected_return, avg_returns
